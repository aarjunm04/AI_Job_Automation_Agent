...
class ChromaStoreConfig:
    host: str = field(default_factory=lambda: os.getenv("CHROMADB_HOST"))
    port: int = field(default_factory=lambda: int(os.getenv("CHROMADB_PORT")))
...
class ChromaStore:
        ...
        while True:
            try:
                ...
                self._collection = self._client.get_or_create_collection(
                    ...
                    metadata={"hnsw:space": os.getenv("CHROMADB_DISTANCE_METRIC", "cosine")}
                )
                logger.info(
    ...
    def get_collection_dimension(self) -> Optional[int]:
        """Discovery helper to find locked dimension of the current collection."""
        if not CHROMADB_AVAILABLE or self._collection is None:
            return None
        try:
            # Peek first result to see embedding length
            data = self._collection.peek(limit=1)
            if data and data['embeddings'] and len(data['embeddings']) > 0:
                dim = len(data['embeddings'][0])
                logger.debug("Discovered collection dimension: %d", dim)
                return dim
        except Exception as e:
            logger.warning("Could not discover collection dimension: %s", e)
        return None
    ...
    def upsert_chunks(
        ...
        if CHROMADB_AVAILABLE and self._collection is not None:
            # ---- DIM GUARD — strictly match collection dimension --------
            coll_dim = self.get_collection_dimension()
            ...
            for i, emb in enumerate(embeddings):
                # If collection empty, allow any valid dim (1024 or 768)
                if coll_dim:
                    if len(emb) != coll_dim:
                        self._logger.error(
                            "DimensionMismatchError: vector is %d, but collection is %d. Skipping.",
                            len(emb), coll_dim
                        )
                        continue
                elif len(emb) not in [1024, 768]:
                    self._logger.error(
    ...
    def add_anchor(
        ...
        coll_dim = self.get_collection_dimension()
        if coll_dim:
            if len(anchor_embedding) != coll_dim:
                self._logger.error(
                    "DimensionMismatchError: anchor is %d, collection is %d. Skipping.",
                    len(anchor_embedding), coll_dim
                )
                return
        elif not anchor_embedding or len(anchor_embedding) not in [1024, 768]:
            self._logger.error(
    ...
    def query(
        ...
        if CHROMADB_AVAILABLE and self._collection is not None:
            # ---- DIM GUARD — query dimension must match collection dimension ----
            coll_dim = self.get_collection_dimension()
            if coll_dim and len(query_embeddings[0]) != coll_dim:
                raise ValueError(
                    f"DimensionMismatchError: Query dimension {len(query_embeddings[0])} "
                    f"does not match index dimension {coll_dim}"
                )
            # --------------------------------------------------------------------
            attempts = 0
            while True:
